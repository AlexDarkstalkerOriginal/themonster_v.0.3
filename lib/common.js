...
var vm = new Vue({
  ...
  data: {      
   ...
   methods: {
    ...
    monster_name: function(id) {
      $('.monstername').trigger('click');
      vm.id_monster_change_name = id;
    }
  }
})  
...
// монстер маркет
    ...
    props: ['speed','name', 'flex', 'power', 'weight', 'src', 'id', 'prize', 'ownername', 'owner', 'view_name', 'newname', 'truename'],
    template: `<div class="monster">\
                <h3 v-if="truename">{{newname}}</h3><h3 v-else>{{name}}</h3>
...
// страница боя
  Vue.component('monster-fight', {
    props: ['speed','name', 'flex', 'power', 'weight', 'src', 'id', 'owner', 'win', 'boolen', 'newname', 'truename'],
    template: `<div class="monster">\
                <h3 v-if="truename">{{newname}}</h3><h3 v-else>{{name}}</h3>
                <img v-bind:src="src" alt="">\
                <a href="#" class="info" v-on:click="vm.monsterPage(id)"><img src="img/info.png" alt=""></a>\
...
// мои монстры лаб  
  Vue.component('my-monster-lab', {
    props: ['speed','name', 'flex', 'power', 'weight', 'src', 'id', 'sale', 'newname', 'truename'],           
    template: `<div class="monster">\
                <div class="name"><h3 v-if="truename">{{newname}}</h3><h3 v-else>{{name}}</h3><a href="#" v-on:click="vm.monster_name(id)" class="edit popup"><img src="img/edit.png" alt=""></a></div>\
                <img v-bind:src="src" alt="">\
                <a href="#" class="info" v-on:click="vm.monsterPage(id)"><img src="img/info.png" alt=""></a>\
                <div class="stats">\
                  <span class="speed">renewal: <span class="value">{{speed}}</span></span>\
                ...
                <span class="weight">{{weight}} lvl</span>\
                <button v-on:click="vm.unsell(id)" v-if="sale" class="unsell"><a>Cancel sell</a></button><button v-on:click="vm.sell(id)" v-else class="sell"><a href="#sell" class="popup">Sell</a></button> \
              </div>`,              
  })
// мои монстры лаб
...
// страница монстра гет
  Vue.component('my-monster-page', {
      props: ['speed','name', 'flex', 'power', 'weight', 'src', 'owner', 'newname', 'truename'],
      template: `<div class="monster">\
                  <h3 v-if="truename">{{newname}}</h3><h3 v-else>{{name}}</h3>
                  <img v-bind:src="src" alt="">\
                  <div class="stats">\
  ...
  function cbMonsterPage(resp) {
     var page_monster = JSON.parse(resp.result);    
      ...
      if (page_monster.idName == page_monster.id) {          
          page_monster.truename = false;
        } else {
            page_monster.newname = page_monster.idName;
          page_monster.truename = true;
        }
...
// монстр батлграунд
  Vue.component('monster-bg', {
    props: ['speed','name', 'flex', 'power', 'weight', 'src', 'id', 'ownername', 'owner', 'view_name', 'newname', 'truename'],
    template: `<div class="monster">\
                <h3 v-if="truename">{{newname}}</h3><h3 v-else>{{name}}</h3>
                <img v-bind:src="src" alt="">\
                <a href="#" class="info" v-on:click="vm.monsterPage(id)"><img src="img/info.png" alt=""></a>\
...
// монстр ранкед
 Vue.component('monster-ranked', {
    props: ['speed','name', 'flex', 'power', 'weight', 'src', 'id', 'owner', 'newname', 'truename'],
    template: `<div class="monster">\
                <div class="name"><h3 v-if="truename">{{newname}}</h3><h3 v-else>{{name}}</h3><a href="#" class="edit"><img src="img/edit.png" alt=""></a></div>\
                <img v-bind:src="src" alt="">\                
                <div class="stats">\
                  <span class="speed">renewal: <span class="value">{{speed}}</span></span>\
                  <span class="agility">def: <span class="value">{{flex}}</span></span>\
...
// мои монстры лаб попап
  Vue.component('my-monster-lab-popup', {
    props: ['speed','name', 'flex', 'power', 'weight', 'src', 'id', 'born_time', 'newname', 'truename'],
    template: `<div class="monster">\
                <div class="name"><h3 v-if="truename">{{newname}}</h3><h3 v-else>{{name}}</h3><a href="#" class="edit"><img src="img/edit.png" alt=""></a></div>\
                <img v-bind:src="src" alt="">\                
                <div class="stats">\
  ...
  function cbMonstersRanked(resp) {
    var monsters_for_ranked = JSON.parse(resp.result);          
      ...
      $.each(monsters_for_ranked,function(index,value){    
        ...
        if (monsters_for_ranked[index].idName == monsters_for_ranked[index].id) {          
          monsters_for_ranked[index].truename = false;
        } else {
          monsters_for_ranked[index].newname = monsters_for_ranked[index].idName;
          monsters_for_ranked[index].truename = true;
        };     
  ...
  function cbMarketMonsters(resp) {
    var market_monsters = JSON.parse(resp.result);          
      ...
      $.each(market_monsters,function(index,value){    
        ...
        market_monsters[index].ownername = market_monsters[index].nameOwner;
        if(market_monsters[index].nameOwner == 'username is not set') {
          market_monsters[index].view_name = false
        } else {
          market_monsters[index].view_name = true;
        };
        if (market_monsters[index].idName == market_monsters[index].id) {          
          market_monsters[index].truename = false;
        } else {
          market_monsters[index].newname = market_monsters[index].idName;
          market_monsters[index].truename = true;
        };
  ...
  function cbMyMonsters(resp) {        
    if (resp.result == '[]') {      
      ...
      $.each(mymonsters_arr,function(index,value){    
        ...
        if (mymonsters_arr[index].idName == mymonsters_arr[index].id) {          
          mymonsters_arr[index].truename = false;
        } else {
          mymonsters_arr[index].newname = mymonsters_arr[index].idName;
          mymonsters_arr[index].truename = true;
        }
...
// история и страница боев обработик 
  ...
  function cbMonsterFight(resp) {    
    ...
    // $.each(history_arr,function(index,value){
     ...
     if (history_arr[vm.fight_id].defender.idName == history_arr[vm.fight_id].defender.id) {          
        history_arr[vm.fight_id].defender.truename = false;
      } else {
        history_arr[vm.fight_id].defender.newname = history_arr[vm.fight_id].defender.idName;
        history_arr[vm.fight_id].defender.truename = true;
      };

     if (history_arr[vm.fight_id].attacker.idName == history_arr[vm.fight_id].attacker.id) {          
        history_arr[vm.fight_id].attacker.truename = false;
      } else {
        history_arr[vm.fight_id].attacker.newname = history_arr[vm.fight_id].attacker.idName;
        history_arr[vm.fight_id].attacker.truename = true;
      };
...
// страница игрока
  ...
  function cbUserPage(resp) {          
    ...
    $.each(player_monsters,function(index,value){    
      ...
      if (player_monsters[index].idName == player_monsters[index].id) {          
        player_monsters[index].truename = false;
      } else {
        player_monsters[index].newname = player_monsters[index].idName;
        player_monsters[index].truename = true;
      };
    ...
    if(player_history.length == 0) {
      ...
      if (vm.player_history_arr.length > 5) {
          console.log('if > 5');
          vm.player_history_arr = vm.player_history_arr.slice(vm.player_history_arr.length-5, vm.player_history_arr.length+1);
      } else {        
      }
...
// изменить имя монстра
  $('.set_mon_name').click(function() {
    var to = dappAddress;
    var value = 0;
    var callFunction = 'setNameMonster';              
    var name = $('#monstername input').val();
    var args = [];    
    args.push(vm.id_monster_change_name);
    args.push(name);
    var callArgs = JSON.stringify(args);    
    nebPay.call(to, value, callFunction, callArgs, { 
      listener: cbChangeNameMonster
    }); 
  })

  function cbChangeNameMonster(resp) {
     hash_value = resp.txhash;    
    if (resp.txhash == undefined) {
     } else {
      $('.transaction').trigger('click');
...
// подсказка чейндж нейм  
  $('.edit_monster_name_tip').hide();
  $(".edit img").hover(function(){
    alert('hover');
    $(this).find('edit_monster_name_tip').show();
    }, function(){
    $(this).find('edit_monster_name_tip').hide();
  });
// подсказка чейндж нейм